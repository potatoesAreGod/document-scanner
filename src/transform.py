...
def sort_coordinates(pts):
    # initialize 2d array for the coordinates
    # TODO: what datatype gives the best precision?
...
def four_point_transform(image, points):
    ...
    # calc width of the new image, which will be max distance between br and bl
    ...
    #                                           the tl and bl y-coordinates
    height_a = np.sqrt(((tr[0] - br[0]) ** 2) + ((tr[1] - br[1]) ** 2))
    height_b = np.sqrt(((tl[0] - bl[0]) ** 2) + ((tl[1] - bl[1]) ** 2))
    height = max(int(height_a), int(height_b))

    # create a top-down view of the document
    dst = np.array([
        [0, 0],
        [width - 1, 0],
        [width - 1, height - 1],
